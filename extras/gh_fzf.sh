...
#     alt-enter   Open a worktree for the selected PR in a new tmux session (tmux only)
#
#   gh-fzf issue
#     alt-w       Open a worktree for the selected issue (new tmux window or inline)
#     alt-enter   Open a worktree for the selected issue in a new tmux session (tmux only)
#
#   gh-fzf run
#     alt-w       Open a worktree for the selected run (new tmux window or inline)
#     alt-enter   Open a worktree for the selected run in a new tmux session (tmux only)
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_pr_opts=(
		...
		"--bind=alt-enter:execute-silent(gh worktree pr {1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/pull-{1})+abort"
	)
else
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_issue_opts=(
		...
		"--bind=alt-enter:execute-silent(gh worktree issue {1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/issue-{1})+abort"
	)
else
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_run_opts=(
		...
		"--bind=alt-enter:execute-silent(gh worktree run {-1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/run-{-1})+abort"
	)
else
