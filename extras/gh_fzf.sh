...
#
#   gh-fzf pr
#     alt-t   Open a worktree for the selected PR (new tmux window or inline)
#     alt-S   Open a worktree for the selected PR in a new tmux session (tmux only)
#
#   gh-fzf issue
#     alt-t   Open a worktree for the selected issue (new tmux window or inline)
#     alt-S   Open a worktree for the selected issue in a new tmux session (tmux only)
#
#   gh-fzf run
#     alt-t   Open a worktree for the selected run (new tmux window or inline)
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_pr_opts=(
		"--bind=alt-t:execute-silent(${_gh_fzf_tmux_cmd} new-window worktrees/pull-{1} gh worktree pr {1})+abort"
		"--bind=alt-S:execute-silent(gh worktree pr {1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/pull-{1})+abort"
	)
else
	_gh_fzf_pr_opts=("--bind=alt-t:execute(gh worktree pr {1})+abort")
fi
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_issue_opts=(
		"--bind=alt-t:execute-silent(${_gh_fzf_tmux_cmd} new-window worktrees/issue-{1} gh worktree issue {1})+abort"
		"--bind=alt-S:execute-silent(gh worktree issue {1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/issue-{1})+abort"
	)
else
	_gh_fzf_issue_opts=("--bind=alt-t:execute(gh worktree issue {1})+abort")
fi
...
if [[ "$_gh_fzf_tmux_use" -eq 1 ]]; then
	_gh_fzf_run_opts=(
		"--bind=alt-t:execute-silent(${_gh_fzf_tmux_cmd} new-window worktrees/run-{-1} gh worktree run {-1})+abort"
		"--bind=alt-S:execute-silent(gh worktree run {-1} --keep -- ${_gh_fzf_tmux_cmd} new-session ${_gh_fzf_repo_name}/run-{-1})+abort"
	)
else
	_gh_fzf_run_opts=("--bind=alt-t:execute(gh worktree run {-1})+abort")
fi
