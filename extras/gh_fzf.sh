...
#
#   gh-fzf pr
#     alt-w   Open a worktree for the selected PR inline
#     alt-W   Open a worktree for the selected PR in a new tmux window  (tmux only)
#     alt-S   Open a worktree for the selected PR in a new tmux session (tmux only)
#
#   gh-fzf issue
#     alt-w   Open a worktree for the selected issue inline
#     alt-W   Open a worktree for the selected issue in a new tmux window  (tmux only)
#     alt-S   Open a worktree for the selected issue in a new tmux session (tmux only)
#
#   gh-fzf run
#     alt-w   Open a worktree for the selected run inline
#     alt-W   Open a worktree for the selected run in a new tmux window  (tmux only)
...
_gh_fzf_pr_opts=(
	'--bind "alt-w:execute(gh worktree pr {1})"'
)
...
_gh_fzf_issue_opts=(
	'--bind "alt-w:execute(gh worktree issue {1})"'
)
if [[ "$_gh_fzf_use_tmux" -eq 1 ]]; then
	_gh_fzf_issue_opts+=(
...
_gh_fzf_run_opts=(
	'--bind "alt-w:execute(gh worktree run {-1})"'
)
if [[ "$_gh_fzf_use_tmux" -eq 1 ]]; then
	_gh_fzf_run_opts+=(
