...
_gh_fzf_pr_opts=(
	'--bind "alt-S:execute-silent(tmux new-session -d -s pull-{1} \"gh worktree pr {1}\" && tmux switch-client -t pull-{1})"'
)
export GH_FZF_PR_OPTS="${GH_FZF_PR_OPTS:+${GH_FZF_PR_OPTS} }${_gh_fzf_pr_opts[*]}"
unset _gh_fzf_pr_opts

_gh_fzf_issue_opts=(
	'--bind "alt-S:execute-silent(tmux new-session -d -s issue-{1} \"gh worktree issue {1}\" && tmux switch-client -t issue-{1})"'
)
export GH_FZF_ISSUE_OPTS="${GH_FZF_ISSUE_OPTS:+${GH_FZF_ISSUE_OPTS} }${_gh_fzf_issue_opts[*]}"
unset _gh_fzf_issue_opts

_gh_fzf_run_opts=(
	'--bind "alt-S:execute-silent(tmux new-session -d -s run-{-1} \"gh worktree run {-1}\" && tmux switch-client -t run-{-1})"'
)
